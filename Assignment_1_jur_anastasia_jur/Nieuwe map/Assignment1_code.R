...
# Result: Named list with the performance measures
# Description: Gives the precision, recall and accuracy for two binary vectors
...
### Data processing and running the function
...
print(table(test_classes, predictions_1))
print(measures_1)
...
print(measures_2)
print(table(test_classes, predictions_2))
...
# c_matrix
#   p1: predctions of first classifier
#   p2: prediction of the second classifier
#   y: gold standard labels
# Description: Creates a contingency matrix from two predictions of two classes
# Returns: The contingency matrix, a matrix
c_matrix <- function(p1, p2, y) {
  yes.yes <- 0 
  yes.no <- 0
  no.yes <- 0 
  no.no <- 0 
  for(i in 1:length(y)){
    # Instances beide correct
    if(((p1[i] == 1 && y[i] == 1) && (p2[i] == 1 && y[i] == 1)) || 
       ((p1[i] == 0 && y[i] == 0) && (p2[i] == 0 && y[i] == 0))) yes.yes = yes.yes +1
    # Instances p1 correct, p2 fout
    if(((p1[i] == 1 && y[i] == 1) && (p2[i] == 0 && y[i] == 1)) || 
       ((p1[i] == 0 && y[i] == 0) && (p2[i] == 1 && y[i] == 0))) yes.no = yes.no + 1
    # Instanc p1 incorrect, p2 correct
    if(((p1[i] == 0 && y[i] == 1) && (p2[i] == 1 && y[i] == 1)) || 
       ((p1[i] == 1 && y[i] == 0) && (p2[i] == 0 && y[i] == 0))) no.yes = no.yes +1
    # Instance p1 and p2 are incorrect
    if(((p1[i] == 0 && y[i] == 1) && (p2[i] == 0 && y[i] == 1)) || 
       ((p1[i] == 1 && y[i] == 0) && (p2[i] == 1 && y[i] == 0))) no.no = no.no +1  
  }
  return(matrix(c(yes.yes, no.yes, yes.no, no.no), nrow = 2))
}

# Create the contingency matrix for each combination of classifiers
c_m_12 <- c_matrix(predictions_1, predictions_2, test_classes)
c_m_13 <- c_matrix(predictions_1, predictions_3, test_classes)
c_m_23 <- c_matrix(predictions_2, predictions_3, test_classes)

# The McNemar's test of classifiers for each matrix
mcnemar_12 <- mcnemar.test(c_m_12)
mcnemar_13 <- mcnemar.test(c_m_13)
mcnemar_23 <- mcnemar.test(c_m_23)
